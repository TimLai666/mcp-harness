...
- 即時控制台：每個 MCP tool call（含唯讀的 `harness`、`project_list` 等）一被呼叫就推 `activity` 事件，Web UI 透過 SSE(`/api/events`)即時顯示「MCP Activity」動態、`terminal_run` 串流輸出、approval/history/project 變更
- 專案 git 狀態：每個專案卡片顯示目前分支與相對 HEAD 的變更行數（`+X -Y`、檔案數、ahead/behind），改檔後即時更新（`GET /api/git`）
- 即時前提：agent 必須連到 **Web server 的 `/mcp`**（與控制台同一個程序），事件才會跨到 dashboard；用獨立的 stdio binary 連線不會反映到 Web UI
...
預設是單一使用者（owner = `local`），所有資料放在 `MCP_HARNESS_HOME` 根目錄，行為與以前相同。設定 OIDC 後即啟用多租戶：

- ChatGPT 的自訂連接器不吃靜態 bearer token，但支援 **OAuth**。設定 OIDC 後，`/mcp` 會變成 OAuth Protected Resource：未帶 token 回 `401` 並指向 `/.well-known/oauth-protected-resource`，該 metadata 宣告 Logto 為 authorization server，ChatGPT 自動發現 Logto 端點、引導使用者登入、拿到 access token（底層仍是 Bearer，只是經 OAuth 流程取得）。
- server 用 Logto 的 JWKS 驗證 JWT（RS256、iss/aud/exp），以 `sub` 當 tenant id。每個工具、history、approval、workspace 都依該 tenant 隔離。
- Web UI 也走 Logto 登入（authorization code）。`/auth/login` → Logto → `/auth/callback` 設 cookie；之後每個 `/api` 只回該使用者自己的資料；live SSE 也只推自己 tenant 的事件。
...
```text
MCP_HARNESS_OIDC_ISSUER=https://<tenant>.logto.app/oidc   # 設了才啟用多租戶
MCP_HARNESS_OIDC_AUDIENCE=<API resource indicator>        # MCP 的 audience（Logto API resource）
MCP_HARNESS_OIDC_CLIENT_ID=<web app client id>           # Web UI 登入用
MCP_HARNESS_OIDC_CLIENT_SECRET=<web app secret>          # 可選（公開 client 可省）
MCP_HARNESS_PUBLIC_URL=https://mcp-harness.example.com    # 對外網址，組 redirect/resource
MCP_HARNESS_SESSION_SECRET=<隨機字串>                     # WebUI cookie 簽章；不設則每次重啟失效
```

Logto 設定：建一個 API resource（indicator = `MCP_HARNESS_OIDC_AUDIENCE`）給 MCP，建一個傳統 Web 應用程式給 WebUI（redirect URI = `<PUBLIC_URL>/auth/callback`）。ChatGPT 端可用 DCR 自動註冊，或在 Logto 預先建 client 後手填。

未經完整 Logto + ChatGPT 線上環境驗證的部分：OIDC 探索、token 交換、ChatGPT 的 OAuth 流程。JWT 驗證、tenant 隔離、metadata、401 challenge、cookie 流程已有單元/整合測試。
...
若要從不可信網路連進來，請設定多租戶 OAuth（見下節），並用 reverse proxy 補 TLS。MCP stdio server 仍保留給本機 MCP host 直接啟動 binary 的情境，不是遠端部署主路徑。

### 上線設定（Logto + `.env`）

機密設定放在 `.env`（已 gitignore；範本見 `.env.example`）。Compose 用 `env_file: .env` 載入，本機 `go run ./cmd/mcp-harness-web` 也會自動讀同目錄的 `.env`。

步驟：

1. Logto 建一個**傳統 Web 應用程式**（WebUI 登入用），Redirect URI 設 `<PUBLIC_URL>/auth/callback`，拿到 App ID / Secret。
2. Logto 建一個 **API Resource**（MCP 用），識別碼填 `<PUBLIC_URL>/mcp`；不需要建任何 permission。
3. 複製 `.env.example` 成 `.env`，填入：

```bash
MCP_HARNESS_OIDC_ISSUER=https://<tenant>.logto.app/oidc   # 結尾 /oidc
MCP_HARNESS_OIDC_CLIENT_ID=<web app id>
MCP_HARNESS_OIDC_CLIENT_SECRET=<web app secret>
MCP_HARNESS_OIDC_AUDIENCE=<PUBLIC_URL>/mcp                # 與 API Resource 識別碼逐字相同
MCP_HARNESS_PUBLIC_URL=<對外網址，無結尾斜線>
MCP_HARNESS_SESSION_SECRET=<openssl rand -hex 32>
```

關鍵：`MCP_HARNESS_OIDC_AUDIENCE`、Logto API Resource 識別碼、`MCP_HARNESS_PUBLIC_URL + /mcp` 這三個字串必須**逐字一致**（差一個斜線或大小寫都會驗不過）。

驗證：開首頁應被導去 Logto 登入，回來後 dashboard 顯示帳號；再去 ChatGPT 連 `<PUBLIC_URL>/mcp` 走 OAuth。沒設 OIDC 時走單一使用者本機模式（owner `local`）。
